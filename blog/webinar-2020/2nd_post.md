...
<span style="color:white">.</span>  
우선, 저희 회사의 경우에는 데이터 엔지니어가 따로 없어요. 그래서 저희는 다른회사에서는 데이터 엔지니어가 해줘야 하는 부분을 개발자랑 분석가가 십시일반 메꾸고 있거든요. 그래서 제가 데이터를 보려면 일단 데이터가 있어야 하는데 유저 행동 데이터 같은건 개발자분들에게 제가 뭔가 요청을 하지 않으면 보고싶은 데이터를 못보더라구요.  
<span style="color:white">.</span>  
